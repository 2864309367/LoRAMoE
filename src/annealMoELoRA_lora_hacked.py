...
class LoraModel(torch.nn.Module):
    ...
    **Attributes**:
    ...
    def _create_new_module(self, lora_config, adapter_name, target, layer_index, number_experts, top_k, key):  ### modified2
        ...
        if loaded_in_8bit and isinstance(target, bnb.nn.Linear8bitLt):
            ...
            new_module = Linear_MoE(adapter_name, in_features, out_features, bias=bias, layer_index=layer_index,
                                number_experts=number_experts, top_k=top_k, key = key, **kwargs)  ### modified2 ##newmodified
    ...
    def _find_and_replace(self, adapter_name):
        ...
        for key in key_list:
            ...
            if isinstance(target, LoraLayer) and isinstance(target, torch.nn.Conv2d):
                ...
                new_module = self._create_new_module(lora_config, adapter_name, target, layer_index, self.number_experts, self.top_k, key) ### modified2
...
class Linear_MoE(nn.Linear, LoraMoE_Layer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # self.expert_activation_count = [0] * self.number_experts
    ...
    def forward(self, x: torch.Tensor):
        ...
        # self.router['router'] 是一个线性层，用于根据输入特征计算每个专家的评分 router_logits
        router_logits = self.router['router'](x)
        # 通过 softmax 转换评分为概率分布
        routing_weights_before = F.softmax(router_logits, dim=1, dtype=torch.float)
        # 使用 topk 筛选最重要的 self.top_k 个专家
        ...
        # 生成每个专家的选择掩码
        '''
        selected_experts 表示被选中的专家索引。
        使用 one_hot 编码，将选择的专家标记为 1，未选择的标记为 0。
        掩码维度为 (number_experts, top_k, batch_size * sequence_length)
        '''
        expert_mask = torch.nn.functional.one_hot(selected_experts, num_classes=self.number_experts).permute(2, 1, 0)



        # 这行代码用于初始化一个形状为 (batch_size * sequence_length, self.out_features) 的零张量，作为最终输出的存储空间
        ...
        for expert_idx in range(self.number_experts):
            ...
            # LoRA 计算：将输入通过该专家的 LoRA 投影层（lora_A 和 lora_B），并根据 scaling 进行放缩
            expert_output = (
            ...
            # 将专家的输出乘以对应的路由权重 routing_weights 例如 0.4*LoRA专家1 + 0.6*LoRA专家3
            current_hidden_states = expert_output * routing_weights[top_x_list, idx_list, None]

            # 使用 index_add_ 将加权后的输出累加到 final_hidden_states
            final_hidden_states.index_add_(0, top_x, current_hidden_states.to(x.dtype))

        # 将专家输出累加到基础线性层结果 result
        ...
        import os
        import json
        if self.layer_i == 0 and self.key.split('.')[-1] == 'q_proj':
            print(f'----------------这是第{self.layer_i}层的{self.key.split(".")[-1]}模块-----------------')
            file_path = "/data/coding/annealMoELoRA/activate_expert_loss_all.json"
            # print(x)
            # print(x.shape)  # 190*4096  190指的是sequence里token数量，4096指的是嵌入向量维度
            # print(selected_experts)
            # print(selected_experts.shape) # 190*2    190指的是sequence里token数量，2指的是选择激活了哪两个专家
            ...
            for i in range(x.shape[0]):
                entry = {
                    "layer": self.layer_i,
                    "module": self.key.split('.')[-1],
                    "token_embedding": x[i].tolist(),  # 将 x 转换为列表
                    "selected_experts": [],  # 将 selected_experts_bin 转换为列表
                    "token_class": ""  # 可以根据需要添加 token_class 的信息
                }
            ...
            if os.path.exists(file_path) and os.path.getsize(file_path) > 0:
                ...
                # 如果文件为空或者不存在，则直接写入新数据
                with open(file_path, "w") as f:
                    json.dump(new_data, f, indent=4)


        if self.layer_i == 28 and self.key.split('.')[-1] == 'q_proj':
            print(f'----------------这是第{self.layer_i}层的{self.key.split(".")[-1]}模块-----------------')
            file_path = "/data/coding/annealMoELoRA/activate_expert_loss_all_expert.json"
            # print(x)
            # print(x.shape)  # 190*4096  190指的是sequence里token数量，4096指的是嵌入向量维度
            # print(selected_experts)
            # print(selected_experts.shape) # 190*2    190指的是sequence里token数量，2指的是选择激活了哪两个专家

             # 将 selected_experts 转换为二进制格式
            selected_experts_bin = torch.zeros((selected_experts.shape[0], 4), dtype=torch.int)

            for i, experts in enumerate(selected_experts):
                selected_experts_bin[i, experts] = 1
            
            # 构建新数据
            new_data = []
            for i in range(x.shape[0]):
                entry = {
                    "selected_experts": selected_experts_bin[i].tolist(),  # 将 selected_experts_bin 转换为列表
                }
                new_data.append(entry)
