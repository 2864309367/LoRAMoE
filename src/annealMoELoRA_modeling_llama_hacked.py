...
#【已修改】newmodified
# 负载均衡损失函数（load_balancing_loss_func）旨在平衡每个专家接收的负载（tokens），避免部分专家过于忙碌或闲置
# 设计逐步退火策略（annealing），使得专家专业性提高，退火策略分为线性退火、指数退火和余弦退火三种方式
# 这个损失函数主要用来调整门控网络
def load_balancing_loss_func(
        ...
        top_k=2,
        current_step: int = 0,
        total_annealing_steps: int = 16500,
        annealing_type: str = "sigmoid",
        alpha: float = 0.02,  # For exponential annealing
        k: float = 0.1,        # For sigmoid annealing
        m: float = 8000        # Midpoint for sigmoid annealing
    ) -> float:
    r"""
    ...
    Args:
        ...
        total_annealing_steps (`int`):
            Total steps over which the annealing factor will decay.
         annealing_type (`str`):
            Type of annealing. Options: ["linear", "exponential", "cosine", "sigmoid"].
        alpha (`float`):
            Decay rate for exponential annealing.
        k (`float`):
    ...
    if annealing_type == "linear":
        annealing_factor = max(0, 1 - current_step / total_annealing_steps)
    elif annealing_type == "exponential":
        annealing_factor = math.exp(-alpha * current_step)
    ...
    def forward(
        ...
        if output_router_logits:
            aux_loss = load_balancing_loss_func(
                router_logits if return_dict else outputs[-1], 
                self.number_experts, 
                self.top_k,
                self.current_step,
            )
        self.current_step = self.current_step + self.current_step_inv    
